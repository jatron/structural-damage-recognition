...
inception_v3 = tf.keras.applications.InceptionV3(input_shape=(224, 224, 3), include_top=True)
# let's visualize layer names and layer indices to see how many layers
# we should freeze:
for i, layer in enumerate(inception_v3.layers):
    print(i, layer.name)
# we will freeze the first 300 layers and unfreeze the rest:
for layer in inception_v3.layers[:313]:
...
history = model.fit(x=X_train_preprocessed, y=y_train, batch_size=100, epochs=15, validation_split=0.1)

pickle.dump(history.history, open("history_freeze_all_layers_incept3_TFP_no_flatten.pkl", "wb"))
