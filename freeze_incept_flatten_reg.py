...
#  tf.keras.layers.GlobalAveragePooling2D(),
  ...
  tf.keras.layers.Dense(len(label_names), activation="softmax",kernel_regularizer=regularizers.l2(0.01))])
...
history = model.fit(x=X_train_preprocessed, y=y_train, batch_size=32, epochs=50, validation_split=0.1)

pickle.dump(history.history, open("history_freeze_all_layers_incept3_add_Dense_Layer_reg.pkl", "wb"))
